...
def calculate_target_share_under_commission(portfolio_value, target_percent, current_price, current_share, comm_func):
    ...
    while True:
        ...
        trading_share = abs(new_share - current_share)
        if new_holding_amount / (portfolio_value - comm_func(trading_share)) <= target_percent:
...
class Trader(object):
    ...
    def update_from_userConfig(self, userConfig):
        ...
        if self._accountCode == '':
            print(__name__ + '::update_from_userConfig: EXIT, do not forget to change accountCode in userConfig')
            exit()
    ...
    def processMessages(self, timeNow):
        ...
        :return:
        """
        self._log.notset(__name__ + '::process_messages: timeNow=%s' % (str(timeNow),))
    ...
    def _validate_input_accountCode(self, accountCode):
        ...
        if accountCode in self.accountCodeCallBackList:
            ...
            self._log.error(
                __name__ + '::_validate_input_accountCode: EXIT, accountCode=%s is not acceptable' % (accountCode,))
            self._log.error(
                __name__ + '::_validate_input_accountCode: possible accountCodes are %s' % (
                    str(self.accountCodeCallBackList),))
            exit()
    ...
    def show_account_info(self, field, meta='value', accountCode='default'):
        ...
        if ans is not None:  # ans could be real 0.0 or 0 for an real account situation
            return ans
    ...
    def get_position(self, security, accountCode='default'):
        ...
        """
        self._log.debug(__name__ + '::get_position: security=%s' % (security.full_print(),))
        adj_accountCode = self.adjust_accountCode(accountCode)
        ...
        for security in adj_securities:
            if not _is_in_security_list(security, ls_excludeSecurities):
                ...
                if holding_quantity > 0:
                    ...
                    ibpyOrderId = self.order(security, -holding_quantity,
    ...
    def rebalance_portfolio(self, traderDecisions, buyAdvantage=0.0, sellAdvantage=0.0, accountCode='default'):
        ...
        for security in traderDecisions:
            ...
            if target_quantity < holding_quantity:
                sellSecurityDict[security] = target_quantity - holding_quantity  # must be negative number
            elif target_quantity > holding_quantity:
                buySecurityDict[security] = target_quantity - holding_quantity  # must be positive number
            else:
            ...
            for security in buySecurityDict:
                ...
                if purchase_amount > 0:
                    try:
                        ...
                        available_fund -= limitPrice * purchase_amount
