...
class BrokerClientBase(IBCpp.IBClient):
    ...
    def _setup(self, log, accountCode, rootFolderPath, singleTrader, dataFromServer, timeGenerator, brokerClientName, syncUuid, autoReconnectPremium=False):
        ...
        self._nextId = Uuid(syncUuid, rootFolderPath=rootFolderPath)  # nextValidId, all request will use the same series
    ...
    def is_real_time_price_requested(self, security):
        self._log.notset(__name__ + '::is_real_time_price_requested: security=%s' % (str(security),))
    ...
    def request_data(self, *args):
        ...
        while not self._activeRequests.check_all_completed():
    ...
    def _send_req_to_server(self, activeRequests):
        ...
        for reqId in activeRequests.get_request_ids():
            ...
            if reqType == 'reqPositions':
                ...
                self.reqMktDataWrapper(reqId, from_security_to_contract(security), genericTickList, snapshot)  # Send market data request to IB server
