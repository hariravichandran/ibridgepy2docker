...
class BrokerClientRobinhood(CallBacks):
    ...
    def setup_this_client(self, log, rootFolderPath, singleTrader, dataFromServer, timeGenerator,
        ...
        self._setup(log, accountCode, rootFolderPath, singleTrader, dataFromServer, timeGenerator, self.name, False)
    ...
    def processMessagesWrapper(self, dummy):
        # Do not delete.
        # This was the old way to deliver data. Good to keep them as a record here
        # self._log.notset(__name__ + '::processMessagesWrapper: dummyTimeNow=%s' % (dummy,))
        # return True
        # self.request_data(ReqPositions(),
        #                   ReqAccountUpdates(True, self.accountCode),
        #                   ReqAllOpenOrders())
        # for str_security in self.realTimePriceRequestedList.getAllStrSecurity():
        #     security = from_string_to_security(str_security)
        #     self.request_data(ReqMktData(security))
        # return True

        # Change to lazy service
        pass
