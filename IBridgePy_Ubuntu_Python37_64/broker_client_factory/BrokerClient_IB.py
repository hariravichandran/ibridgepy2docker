...
class ClientIB(CallBacks):
    ...
    def setup_this_client(self, log, accountCode, rootFolderPath, singleTrader, dataFromServer, timeGenerator, host, port, clientId, syncOrderId, autoReconnectPremium):
        self._setup(log, accountCode, rootFolderPath, singleTrader, dataFromServer, timeGenerator, self.name, syncOrderId, autoReconnectPremium)
    ...
    def connectWrapper(self):
        self._log.notset(__name__ + '::connectWrapper')
        self.connect(self.host, int(self.port), int(self.clientId))

    def disconnectWrapper(self):
        self._log.notset(__name__ + '::disconnectWrapper')
        self.disconnect()
